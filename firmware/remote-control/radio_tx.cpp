...
public:
    ...
    void setTxMode(const uint8_t *txAddress) override
    {
        ...
        writeRegister(NRF_REG_RX_PW_P0, sizeof(Payload)); // 11 bytes payload size (matches struct Payload)
    ...
    bool sendPayload(const void *buf, uint8_t size) override
    {
        ...
        // 6. If MAX_RT (transmission failed), we must flush the TX FIFO
        if (status & (1 << 4))
        {
            flushTx();
            return false;
...
static Atm328Gpio cePin(&DDRB, &PORTB, &PINB, PORTB1); // CE connected to PB1 (Digital Pin 9)
...
namespace RadioTx
{
    ...
    {
        return nrf24.sendPayload(&data, sizeof(Payload));
    }
}
